...
function getWorkerByUserId(userId: number) {
	return db.prepare('SELECT * FROM workers WHERE user_id = ?').get(userId) as any;
}
...
app.post('/api/auth/register', (req, res) => {
	...
	if (role === 'worker') {
		...
		db.prepare('INSERT INTO workers (user_id, active) VALUES (?,1)').run(info.lastInsertRowid as number);
	}
...
app.post('/api/admin/workers', authMiddleware, requireRole('admin'), (req, res) => {
	...
	db.prepare('INSERT INTO workers (user_id, phone, active) VALUES (?,?,1)').run(info.lastInsertRowid as number, phone ?? null);
...
app.post('/api/reports', authMiddleware, requireRole('worker'), (req: any, res) => {
	...
	const info = db.prepare(`INSERT INTO reports (worker_id, customer_name, customer_address, issue_description, visit_date, expenses, amount_collected, amount_due, notes) VALUES (?,?,?,?,?,?,?,?,?)`).run(
		worker.id,
		customer_name,
...
app.get('/api/reports/my', authMiddleware, requireRole('worker'), (req: any, res) => {
...
app.patch('/api/reports/:id', authMiddleware, (req: any, res) => {
	...
	const row = db.prepare('SELECT r.*, w.user_id FROM reports r JOIN workers w ON w.id = r.worker_id WHERE r.id = ?').get(id) as any;
	...
	db.prepare('UPDATE reports SET expenses = COALESCE(?, expenses), amount_collected = COALESCE(?, amount_collected), amount_due = ?, notes = COALESCE(?, notes) WHERE id = ?')
		.run(
			typeof expenses === 'number' ? expenses : null,
			typeof amount_collected === 'number' ? amount_collected : null,
			amount_due,
			notes ?? null,
			id
		);
